...
async function cargarServicio(tipo) {
    ...
    // Spinner de carga
    contenedor.innerHTML = '<div class="col-12 text-center py-5"><div class="spinner-border text-danger"></div><p class="mt-2 text-white">Buscando ofertas...</p></div>';
    ...
    try {
        ...
        if (!datos.length) { 
            ...
            <div class="col-12 text-center py-5">
                ...
                <h4 class="text-white">No hay resultados disponibles</h4>
            </div>`; 
            return; 
        }
        ...
        datos.forEach(item => {
            ...
            // 1. Configurar datos según el tipo (Vuelos, Hoteles, Autobuses)
            if (tipo === 'vuelos') {
            ...
            // 2. Crear el Badge de la Agencia (NUEVO)
            // Si el backend no devuelve nombre_agencia, usa un fallback
            const nombreAgencia = item.nombre_agencia || 'ViajeGO Oficial';
            const badgeAgencia = `
                <div class="position-absolute top-0 end-0 m-2 badge bg-dark border border-secondary text-light shadow-sm" style="z-index: 2;">
            ...
            html += `
            <div class="col">
                <div class="card h-100 bg-black border border-secondary text-white shadow-sm position-relative card-hover">
                    ${badgeAgencia} <div class="card-body mt-2">
                        ...
                        <p class="card-text mb-1 fw-bold fs-5">${desc}</p>
                        <div class="mb-3">${detalles}</div>
                        
                        <div class="d-flex justify-content-between align-items-end mt-auto pt-3 border-top border-secondary border-opacity-50">
                            <div>
                                <small class="text-secondary text-uppercase" style="font-size: 0.7rem;">Precio desde</small>
                                <h4 class="fw-bold text-success mb-0">$${parseFloat(precio).toLocaleString()}</h4>
                            </div>
                            <button class="btn btn-outline-danger btn-sm fw-bold px-3" onclick="abrirModal('${tipo}', '${itemString}')">
                                Ver Detalles
                            </button>
                        </div>
                    </div>
        ...
        console.error(e);
        contenedor.innerHTML = '<div class="col-12 text-center text-danger py-5">Error al cargar los datos.</div>';
    }
}
